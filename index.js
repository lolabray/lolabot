...
fetch("https://api.lovense.com/api/lan/getToys").then(async response => {
	...
	if (keys.length === 0) {
		...
		console.error("Multiple connected users on same network, currently not setup to support this.");
		process.exit(1);
	}
	...
	for (const [toyId, toyDetails] of Object.entries(lovenseDetails.toys)) {
		console.log(`${toyDetails.nickName}: (${toyDetails.id})  [Version: ${toyDetails.version}] [Battery: ${toyDetails.battery}]     ${toyDetails.status === 1 ? "Connected" : "Disconnected"}`);
	...
	client.on('interactionCreate', async (interaction) => {
		...
		switch (commandName) {
			...
			case "battery":
				...
				await fetch(lovenseUrl(
					"Battery",
					{t: options.getString('toy') }
				))
				.then(value => {
      ...
      case 'stop':
				...
				var params = {
          sec: 0,
					v: (commandName === "high" 		? 20
						: commandName === "medium"  ? 10
            : commandName === "low"     ? 1
						: 0)	// Stop
				}
				if (toy !== "ALL") { params.t = toy; }
				await fetch(lovenseUrl(
					'AVibrate',
					params
				));
      ...
      case 'grind':
        var toy = options.getString('toy');
				...
				var params = {
          sec: 0,
					v: (commandName === "pulse" 	? 1
						: commandName === "circle"  ? 2
            : commandName === "grind"   ? 3
						: 0)	// Something?
				}
				if (toy !== "ALL") { params.t = toy; }
...
.catch(error => {
	console.error("Failed to load teh Lovense API for Toy Information. This can be due to many reasons.");
...
function lovenseUrl(endpoint, values) {
	return `https://${domain}:${port}/${endpoint}`+
